...
void loop() {
    ...
    if (!readLine(line, overflowed)) {
        if (overflowed) sendError("unknown", "ERR_LINE_TOO_LONG", "input line too long");
        return;
    }
    line.trim();
    ...
    // ------------------------------------------------------------
        ...
        if (doc.containsKey("brightness")) {
            ...
            if (br >= 0 && br <= 255) currentBrightness = static_cast<uint8_t>(br);
        }
        ledOn = true;
        applyLed();
        StaticJsonDocument<128> res;
        ...
        currentBrightness = static_cast<uint8_t>(br);
        applyLed();
        StaticJsonDocument<96> res;
        res["status"] = "ok";
        res["brightness"] = currentBrightness;
